...
public class WBannerController {
     ...
     * 表:w_banner ===> 所有:带条件
     */
    @GetMapping(value = "/findAllWBanner")
    public void findAllWBanner(HttpServletResponse response, String jsonStr, SelectEntity selectEntity) {
     ...
     * 分页条件查询
     */
    @GetMapping(value = "/findPageWBanner")
    public void findPageWBanner(HttpServletResponse response,String jsonStr,  SelectEntity selectEntity) {
        PageRequest pageRequest = JpaUtils.getPageRequest(selectEntity);
        Page<WBanner> page = wBannerService.findPage(jsonStr, pageRequest);
        ResponseUtils.responseJsonFilter(response, PageUtil.pageFormat(page),WBannerConst.filterselectColumnArr);
    }

    /**
     * 根据ID查询
     */
    @GetMapping(value = "/findIdWBanner")
    public void findIdWBanner(HttpServletResponse response,@RequestParam Integer id) {
        WBanner obj = wBannerService.findId(id);
        ResponseUtils.responseJsonFilter(response, obj,WBannerConst.filterselectColumnArr);
    }
     ...
     * 添加或修改
     */
    @PostMapping(value = "/saveWBanner")
    public void saveWBanner(HttpServletResponse response,@RequestBody WBanner wBanner) {
        WBanner obj = null;
        //添加
        ...
        if (null == id) {
            //添加
            log.info("添加:{}", wBanner);
            WhyBeanUtils.filterField(wBanner, WBannerConst.filteraddColumnArr);
            wBanner.setCreatetime(new Date());
            wBanner.setTid(WhyStringUtil.getUUID());
            obj = wBannerService.save(wBanner);
        } else {
            //修改
            log.info("修改:{}", wBanner);
            WBanner wBanner1 = wBannerService.findId(id);
            Assert.notNull(wBanner1, "操作失败！");
            WhyBeanUtils.filterField(wBanner, WBannerConst.filteraddColumnArr);
            obj = wBannerService.save(wBanner);
        }
        ResponseUtils.responseJsonFilter(response, obj,WBannerConst.filterselectColumnArr);
     ...
     * 删除
     */
    @PostMapping(value = "/delWBanner")
    public Map delWBanner(@RequestBody Integer idArr[]) {
