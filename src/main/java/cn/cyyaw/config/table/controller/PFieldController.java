...
import cn.cyyaw.config.table.table.entityconst.PFieldConst;
...
public class PFieldController {
     ...
     * 表:p_field ===> 所有:带条件
     */
    @GetMapping(value = "/findAllPField")
    public void findAllPField(HttpServletResponse response, String jsonStr, SelectEntity selectEntity) {
        List<PField> list = pFieldService.findAll(jsonStr, selectEntity);
        ResponseUtils.responseJsonFilter(response, list, PFieldConst.filterselectColumnArr);
    }

    /**
     * 分页条件查询
     */
    @GetMapping(value = "/findPagePField")
    public void findPagePField(HttpServletResponse response,String jsonStr,  SelectEntity selectEntity) {
        PageRequest pageRequest = JpaUtils.getPageRequest(selectEntity);
        Page<PField> page = pFieldService.findPage(jsonStr, pageRequest);
        ResponseUtils.responseJsonFilter(response, PageUtil.pageFormat(page),PFieldConst.filterselectColumnArr);
    }

    /**
     * 根据ID查询
     */
    @GetMapping(value = "/findIdPField")
    public void findIdPField(HttpServletResponse response,@RequestParam Integer id) {
        PField obj = pFieldService.findId(id);
        ResponseUtils.responseJsonFilter(response, obj,PFieldConst.filterselectColumnArr);
    }
     ...
     * 添加或修改
     */
    @PostMapping(value = "/savePField")
    public void savePField(HttpServletResponse response,@RequestBody PField pField) {
        PField obj = null;
        //添加
        Integer id = pField.getId();
        if (null == id) {
            //添加
            log.info("添加:{}", pField);
            WhyBeanUtils.filterField(pField, PFieldConst.filteraddColumnArr);
            pField.setCreatetime(new Date());
            pField.setTid(WhyStringUtil.getUUID());
            obj = pFieldService.save(pField);
        } else {
            //修改
            log.info("修改:{}", pField);
            PField pField1 = pFieldService.findId(id);
            Assert.notNull(pField1, "操作失败！");
            WhyBeanUtils.filterField(pField, PFieldConst.filteraddColumnArr);
            obj = pFieldService.save(pField);
        }
        ResponseUtils.responseJsonFilter(response, obj,PFieldConst.filterselectColumnArr);
     ...
     * 删除
     */
    @PostMapping(value = "/delPField")
    public Map delPField( @RequestBody Integer idArr[]) {
        pFieldService.del(idArr);
